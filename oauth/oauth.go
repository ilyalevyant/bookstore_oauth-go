...
import (
	"encoding/json"
	"fmt"
	"github.com/ilyalevyant/bookstore_oauth-go/oauth/errors"
...
func getAccessToken(accessTokenId string) (*accessToken, *errors.RestErr) {
	response := oauthRestClient.Get(fmt.Sprintf("/oauth/access-token/%s", accessTokenId))
	...
	if response.StatusCode > 299 {
		...
		if err != nil{
			return nil, errors.NewInternalServerError("invalid error interface get access token")
		}
		return nil, &restErr
	}
